...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.user.isAdmin) {
        ...
        const roles = organizations.map(
          (membership) => membership.role.toLowerCase()
        );
        const hasOfficer = roles.includes("officer");
        const hasUser = roles.includes("user");

        // If the user has both roles, show the OrgLoginModal
        if (hasOfficer && hasUser) {
          setLoggedInUser(data.user);
          setShowModal(true);
        } 
        // If the user only has officer role, navigate to the officer dashboard
        else if (hasOfficer) {
  ...
  // A helper to close the modal (if needed after navigation)
  const closeModal = () => {
    setShowModal(false);
  };
